...
hierarquia$cod_municipiodv <- as.character(hierarquia$cod_municipiodv)

cd_oferta_vs_demanda_regiao_saude <- 
  cd_oferta_vs_demanda_br |>
    left_join(hierarquia, by = c("cod_mun_loc"="cod_municipiodv")) |> 
...
### 5.1.1. Plano de saúde

Também é possível fazer um exercício para deduzir uma parcela da população que é beneficiária de plano odontológico e eventualmente utilizaria serviços privados.
...
plano_odontologico <-    
                      plano_de_saude |>      
                            ...
                            filter(FAIXA_CORRIGIDA != "Inconsistente") |>      
                            mutate(taxa = (PLANO_ODONTO/POP) * 100) |>                             filter(taxa < 100) |>      
                            mutate(id_faixa = case_whe(FAIXA_CORRIGIDA == "de 0 a 14 anos" ~ 1,                        
...
pop_brasil_tratado <-    
  pop_brasil_tratado |>    
  left_join(plano_odontologico, by = c("ibge"="CD_MUNICIPIO",                                        "id_faixa")) 

pop_brasil_tratado$taxa[is.na(pop_brasil_tratado$taxa)] <- 0     pop_brasil_tratado <- pop_brasil_tratado |>    
  mutate(pop_dependente = total - (total * (taxa/100)))   
```
