...
oferta_vs_demanda_br$FTE_40_linha[is.na(oferta_vs_demanda_br$FTE_40_linha)] <- 0


oferta_vs_demanda_br$FTE40[is.na(oferta_vs_demanda_br$FTE40)] <- 0


oferta_vs_demanda_br$FTE_40_direto[is.na(oferta_vs_demanda_br$FTE_40_direto)] <- 0
...
mun2 <- oferta_vs_demanda_br |> 
    left_join(mun, by= c("cod_mun_loc" = "code_muni"))
  
mun_sf <- st_as_sf(mun2)

ggplot() +
  geom_sf(data = mun_sf, aes(fill = rr), color = NA) +
  scale_fill_gradientn(colors = c("#D92B3A", "#F2C744", "#ADBF69"), 
                       values = rescale(c(0, 100, 600)), 
                       limits = c(0, 600)) +  
