...
spdf <- geojson_read("C:/Users/alefs/OneDrive/Documentos/saude_bucal/02_script/BR_Regionais_Simplificado.geojson", what = "sp")
...
oferta$cod_macrorregiao <- as.integer(oferta$cod_macrorregiao)
...
capitais_coord <- 
  oferta_vs_demanda |>
  mutate(ibge = as.character(ibge)) |> 
  filter(ibge %in% capitais) |> 
  select(ibge, municipio.x, longitude, latitude) |> 
...
# Criar o mapa
mapa <- spdf_fortified |>
  left_join(oferta, by = c("reg_id"="cod_macrorregiao")) |>
  ggplot() +
...
# gerando mapa
  ...
  mun2 <- left_join(oferta_vs_demanda, mun, by = c("cod_mun_loc" = "code_muni")
...
#Incorporando a coluna Geom do data municipios no dataframe oferta_vs_demanda



oferta_vs_demanda2 <- oferta_vs_demanda |> 
  left_join(municipios, by = c("cod_municipiodv" = "code_muni")) |>    select(geom, ibge, municipio.x, nivel, cod_municipiodv, necessidade, CATEGORIA, FTE40, FTE_40_direto, FTE_40_linha, ra, rr)
