...
We will present two applications (A and B). In part A, for didactic purposes, presented the application for just one municipality. Following that, in section B, we expanded the application to the entire national territory.

# A) Reduced Application for One Municipality

## 1. Scope Definition

The methodology aims to estimate the current and required workforce of dentists, dental technicians, and oral health assistants for primary care services in just one municipality (Palmas-TO). This includes individual procedures as well as three specialized procedures: endodontics, prosthodontics, and periodontics.

## 2. Oral Health Needs Analysis

The oral health needs analysis (OHNA) was constructed based on three elements: population distribution by age group (P), prevalence of oral health conditions (H), and procedures per capita (S).
...
The age group distribution was built based on data from the Brazilian latest census (IBGE, 2022). As mentioned before, we will focus on just one municipality as an example: Palmas (TO).
...
### 2.2. Oral health coverage based in some conditions (H)

The Brazilian Ministry of Health provides several parameters regarding oral health (Brasil, 2021). One of these parameters is the coverage of oral health services, calculated according to Brazil's most importante oral health survey (SB 2010). There are parameters for each age group and type of procedure, as explained below.
...
The coverage for each age group .
...
To access the supply of professionals, we accessed the National Register of Health Establishments - Professionals Table (*Cadastro Nacional de Estabelecimentos de Saúde*, CNES-PF, in portuguese). Since professionals may have multiple affiliations, the counting was standardized using Full-time Equivalent (FTE), based on the professional's contracted workload.
...
Finally, it is suggested to calculate the financial impact of this gap. The average salary of dental surgeons in Palmas-TO is R\$ 4000 according to Continuous National Household Sampling Survey (PNADc) of the Brazilian Institute of Geography and Statistics (IBGE). We added a multiplicative factor for labor costs (1.42). These procedure result in the following table.
...
### 5.2. Oral health coverage based on conditions (H) - Brazil

The script below is used to read the oral health coverage (H) for each age group across all municipalities in Brazil.

The capitals of all states have associated coverage values. However, municipalities in the countryside do not have individualized coverage but rather a parameter corresponding to the region.

The municipality of Formosa is located in the state of Goiás, which is part of the Midwest Region, so we use the parameter for the countryside of the Midwest Region).
...
```{r}
rend_medio <- read_excel("C:/Users/alefs/OneDrive/Documentos/saude_bucal/01_dados/rendimento_saudebocal.xlsx")
