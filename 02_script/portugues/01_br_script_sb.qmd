...
cd_oferta_vs_demanda_regiao_saude <- 
  cd_oferta_vs_demanda_br |>
    ...
    mutate(rr_s = if_else(rr_s == 0.00, 1.00, rr_s))
...
ggsave(filename = "mapa_aps.jpeg", plot = a,
       dpi = 400, width = 14, height = 9)
...
mapa_rs |> 
  group_by(nivel, 
           uf_sigla) |> 
  summarise(nec = sum(necessidade),
            of = sum(oferta)) |> 
  mutate(rr = 100 * (of/nec)) |> 
  mutate(rr = round(rr, 1)) |> 
  filter(nivel == "AES") |> 
  ggplot(aes(x = fct_reorder(uf_sigla, 
                             rr), 
             y = rr)) +
  geom_col() + 
...
#### 7.3.1.1.1 Resultados por população SUS
